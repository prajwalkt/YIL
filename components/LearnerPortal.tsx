                    ...
                    {e.TrainerName && (
                      ...
                      <div className="mt-auto pt-4">
                        ...
                        <div className="flex gap-2 mb-4">
                          <button onClick={() => setActiveTab('assessments')} className="flex items-center gap-1 text-[10px] font-bold text-blue-600 bg-blue-50 px-2 py-1 rounded border border-blue-100 hover:bg-blue-100 transition-colors"><ClipboardList size={10}/> Assessments</button>
...
function ELearningViewer({ setActiveTab }: { setActiveTab?: (tab: string) => void }) {
  ...
  if (activeContent && activeCourse) return (
    <div className="space-y-4">
      ...
      <div className="bg-black rounded-2xl overflow-hidden shadow-2xl relative select-none">
        {activeContent.ContentType === 'VIDEO' ? (
          <video
            ref={videoRef}
            key={activeContent.ContentID}
            src={activeContent.FilePath}
            controls
...
function FeedbackTab() {
  ...
  const submitFeedback = async () => {
    ...
    const d = await res.json();
    setMsg(d.message);
    if (d.success) { setActiveFeedback(null); load(); }
    setSubmitting(false);
  };
